...
namespace GuardianStock.Application.Products.Commands.BulkImportProducts
{
    ...
    public class BulkImportProductsCommandHandler : IRequestHandler<BulkImportProductsJob>
    {
        ...
        public async Task Handle(BulkImportProductsJob request, CancellationToken cancellationToken)
        {
            ...
            var userIdString = request.UserId?.ToString() ?? string.Empty;
            ...
            {
                await _signalService.OnJobFailed(userIdString, request.JobId, "The preview is missing, please upload a csv again.", cancellationToken);
            ...
            await _signalService.SendOnProgress(userIdString, request.JobId, 100.0m, "Import is completed.", cancellationToken);

            await _signalService.SendOnImportCompleted(userIdString, importSummary, cancellationToken);
