...
namespace GuardianStock.Infrastructure.FileManager;
...
public class FileManagerService : IFileManagerService
{
    ...
    public string? GetFilePath(string fileName, string directory)
    {
        ...
        return File.Exists(fileFullPath) ? fileFullPath : null;
    }
    ...
    public async Task<string> UploadFileAsync(IFormFile file, string directory, string fileName, CancellationToken cancellationToken = default)
    {
        ...
        var path = Path.Combine(hostEnvironment.ContentRootPath, directory);
